...
#pragma omp for
  ...
  std::cerr << "Map update took " << t2 - t1 << " seconds for " << boxes.size()
            << " constraints\n";
}
...
#pragma omp for
        ...
        for (id.k = 0; id.k < size_z; ++id.k) {
          ...
          for (int i = 0; i < valid_configs.size(); ++i) {
            if (valid_configs[i]->palm(x) || valid_configs[i]->fingerSweep(x)) {
              grid[id.i][id.j][id.k] = SimpleOccMap::OCC;
              config_grid[id.i][id.j][id.k].configs.push_back(
                  &valid_configs[i]);
  ...
  std::cerr << "Gripper map update took " << t2 - t1 << " seconds for "
            << valid_configs.size() << " constraints\n";
}

void ConstraintMap::getConfigsForDisplay(
    pcl::PointCloud<pcl::PointXYZRGB> &configs_pc) {
  CellIndex id;
  Eigen::Affine3f pose;
  ...
  // printf("bounds were: %d,%d adjusted are: %d,%d
  // no=%d",cube.bl.j,cube.ur.j,lowY,highY,n_o);
      ...
      for (id.k = 0; id.k < n_d; ++id.k) {
        ...
        if (valid_configs[config_index]->isValid) {
          ...
          if ((id.k >= cube.bl.k && id.k <= cube.ur.k) &&  // non-looped degreed
              (isSphereGrid ? (lowX <= highX ? (id.i >= lowX && id.i <= highX)
                                             : (id.i >= lowX || id.i <= highX))
                            : (id.i >= cube.bl.i && id.i <= cube.ur.i)) &&
              (lowY <= highY ? (id.j >= lowY && id.j <= highY)
                             : (id.j >= lowY || id.j <= highY))) {
            // selected
            pc.rgb = select.rgb;
          } else {
            // not selected
          ...
          if( ( id.k >= cube.bl.k && id.k<=cube.ur.k) && //non-looped degreed
              ( isSphereGrid ? (lowX<=highX ? (id.i >= lowX && id.i<= highX) :
          (id.i >= lowX || id.i<= highX )) : (id.i >= cube.bl.i &&
          id.i<=cube.ur.i) ) &&
              (  lowY<=highY ? (id.j >= lowY && id.j<= highY) :  (id.j >= lowY
          || id.j<= highY ) ) ) {
            } */
...
void ConstraintMap::generateOpeningAngleDump(std::string &fname) {
  ...
  fprintf(fout, "bounds = [%d, %d, %d; %d, %d, %d];\n", cube.bl.i, cube.bl.j,
          cube.bl.k, cube.ur.i, cube.ur.j, cube.ur.k);
  fclose(fout);
}
...
bool ConstraintMap::getPoseForConfig(CellIndex &id, Eigen::Affine3f &pose) {
  ...
  if (!isSphereGrid) {
    ...
    pose = Eigen::AngleAxisf(M_PI + 2 * id.j * M_PI / n_o,
    ...
    pose = Eigen::AngleAxisf(M_PI / 2 + phi, Eigen::Vector3f::UnitZ()) *
           Eigen::AngleAxisf((3 * M_PI / 2 + theta), Eigen::Vector3f::UnitX());
...
void ConstraintMap::sampleGripperGridSphere(int n_vert_angles,
                                            int n_orient_angles,
                                            int n_dist_samples, float min_dist_,
                                            float max_dist_) {
  isSphereGrid = true;
  n_v = n_vert_angles;
  n_o = n_orient_angles;
  ...
  for (int i = 0; i < n_v; ++i) {
    ...
    for (int j = 0; j < n_o; ++j) {
      ...
      for (int k = 0; k < n_d; ++k) {
        ...
        Eigen::Affine3f pose;
        // pose = Eigen::AngleAxisf(M_PI-2*j*M_PI/n_o,
        // Eigen::Vector3f::UnitZ())*Eigen::AngleAxisf(M_PI-2*i*M_PI/n_v,
        // Eigen::Vector3f::UnitX());
        pose =
            Eigen::AngleAxisf(M_PI / 2 + phi, Eigen::Vector3f::UnitZ()) *
            Eigen::AngleAxisf((3 * M_PI / 2 + theta), Eigen::Vector3f::UnitX());
        // pose.setIdentity();
        pose.translation()(0) = r * sinf(theta) * cosf(phi);
        pose.translation()(1) = r * sinf(theta) * sinf(phi);
        ...
        if (isPJGripper && modelPJ != NULL) {
          GripperConfigurationPJ *config =
              new GripperConfigurationPJ(pose, modelPJ);
          config->calculateConstraints();
          valid_configs.push_back(config);
        }
        // valid_configs.size () = k + j*n_dist_samples +
        // i*n_orient*n_dist_samples
      }
    }
  }
}

void ConstraintMap::sampleGripperGrid(int n_vert_slices, int n_orient,
                                      int n_dist_samples, float min_z_,
                                      float max_z_, float min_dist_,
                                      float max_dist_) {
  n_v = n_vert_slices;
  n_o = n_orient;
  n_d = n_dist_samples;
  ...
  for (int i = 0; i < n_vert_slices; ++i) {
    for (int j = 0; j < n_orient; ++j) {
      for (int k = 0; k < n_dist_samples; ++k) {
        ...
        Eigen::Affine3f pose;
        pose = Eigen::AngleAxisf(2 * j * M_PI / n_orient,
                                 Eigen::Vector3f::UnitZ());
        Eigen::Vector3f ori = Eigen::Vector3f::UnitY();
        ori = pose * ori;
        ori *= (min_dist + ((max_dist - min_dist) * k) / n_dist_samples);
        ori(2) = z;
        pose = Eigen::AngleAxisf(M_PI + 2 * j * M_PI / n_orient,
                                 Eigen::Vector3f::UnitZ());
        pose.translation() = ori;
...
void ConstraintMap::computeValidConfigs(SimpleOccMapIfce *object_map,
                                        Eigen::Affine3f cpose, float cradius,
                                        float cheight,
                                        Eigen::Vector3f &prototype_orientation,
                                        float orientation_tolerance,
                                        GripperPoseConstraint &output) {
  ...
  for (int i = 0; i < valid_configs.size(); ++i) {
    ...
    Eigen::Vector3f zaxis =
        valid_configs[i]->pose.rotation() * Eigen::Vector3f::UnitY();
    float angle = acosf(prototype_orientation.dot(zaxis.normalized()));
    valid_configs[i]->isValid =
        (orientation_tolerance < 0) || (angle < orientation_tolerance);
    /*
    ...
    //if(diff > M_PI) diff -= M_PI;	  //treat orientations equally in both
    directions
    //note orientation_tolerance < 0 => disable orientation bias
    valid_configs[i]->isValid = (orientation_tolerance < 0) || ( diff <=
        ...
        if (*config_grid[id.i][id.j][id.k].configs[j] != NULL) {
          if ((*config_grid[id.i][id.j][id.k].configs[j])->isValid) {
            if (!isSphereGrid) {
              ...
              if ((*config_grid[id.i][id.j][id.k].configs[j])->max_oa <=
                      (*config_grid[id.i][id.j][id.k].configs[j])->min_oa ||
                  (*config_grid[id.i][id.j][id.k].configs[j])->palm(x)) {
                // if max_oa < min_oa, or if intersect with palm, -> config is
                // not valid delete config
  ...
  for (int i = 0; i < valid_configs.size(); ++i) {
    if (valid_configs[i] != NULL) {
      ...
      if (valid_configs[i]->isValid && valid_configs[i]->min_oa > 0) {
        ...
        // std::cerr<<"id "<<i<<" is "<<id.i<<","<<id.j<<","<<id.k<<" ?== "<<(i
        // == id.k + n_d*id.j + n_d*n_o*id.i)<<std::endl;
        CellIndex id;
        id.k = i % n_d;
        id.j = ((i - id.k) / n_d) % n_o;
  ...
  double t2 = getDoubleTime();
  std::cerr << "Had " << valid_configs.size() << " configs, now we have "
            << valid_configs2.size() << " and it took " << t2 - t1
            << " seconds\n";
  ...
  std::cout << "MAX cube at (" << cube.bl.i << "," << cube.bl.j << ","
            << cube.bl.k << ") : (" << cube.ur.i << "," << cube.ur.j << ","
            << cube.ur.k << ") size: " << xlen << "," << ylen << " volume "
            << volume << " cvolume " << cube.volume() << std::endl;
      ...
      for (id.k = cube.bl.k; id.k <= cube.ur.k; ++id.k) {
        ...
        if (valid_configs[v] != NULL) {
          if (valid_configs[v]->isValid) {
            // std::cout<<v<<" "<<id.i<<" "<<id.j<<" "<<id.k<<"
            // "<<valid_configs[v]->min_oa<<"
            // "<<valid_configs[v]->max_oa<<std::endl;
            output.min_oa = valid_configs[v]->min_oa > output.min_oa
                                ? valid_configs[v]->min_oa
                                : output.min_oa;
            output.max_oa = valid_configs[v]->max_oa < output.max_oa
                                ? valid_configs[v]->max_oa
                                : output.max_oa;
          }
        }
  ...
  std::cout << "min " << output.min_oa << " max " << output.max_oa
            << " checked " << dctr << std::endl;
  ...
  if (!isSphereGrid) {
    ...
    Eigen::Affine3f pose;
    pose =
        Eigen::AngleAxisf(2 * cube.bl.j * M_PI / n_o, Eigen::Vector3f::UnitZ());
    Eigen::Vector3f ori = Eigen::Vector3f::UnitY();
    ori = pose * ori;
    ...
    pose.setIdentity();
    output.inner_cylinder = CylinderConstraint(
        pose, (min_dist + ((max_dist - min_dist) * cube.bl.k) / n_d),
        max_z - min_z);

    // upper right gives upper plane, right plane and inner cylinder
    pose =
        Eigen::AngleAxisf(2 * cube.ur.j * M_PI / n_o, Eigen::Vector3f::UnitZ());
    ...
    pose.setIdentity();
    output.outer_cylinder = CylinderConstraint(
        pose, (min_dist + ((max_dist - min_dist) * cube.ur.k) / n_d),
        max_z - min_z);
  } else {
    ...
    pose = Eigen::AngleAxisf(M_PI/2+phi_midpoint,
    Eigen::Vector3f::UnitZ())*Eigen::AngleAxisf(theta,
    Eigen::Vector3f::UnitX());
    ori = pose*Eigen::Vector3f::UnitY();
  ...
  for (id.i = 0; id.i < size_x; ++id.i) {
    for (id.j = 0; id.j < size_y; ++id.j) {
      for (id.k = 0; id.k < size_z; ++id.k) {
        ...
        fwrite(config_grid[id.i][id.j][id.k].config_ids.data(), sizeof(int), sz,
               fout);
      }
    }
  }
...
bool ConstraintMap::loadGripperConstraints(const char *fname) {
  ...
  if (strncmp(versionBuf, _FILE_VERSION_, strlen(_FILE_VERSION_)) != 0) {
    if (strncmp(versionBuf, _FILE_VERSION_IROS2016,
                strlen(_FILE_VERSION_IROS2016)) != 0) {
      std::cerr << "Version mismatch, don't know what to do: " << versionBuf
                << std::endl;
      return false;
    } else {
  ...
  std::cerr << "read c " << center.transpose() << " s "
            << size_meters.transpose() << " r " << resolution << std::endl;
  std::cerr << "collision map size is " << size_x << " " << size_y << " "
            << size_z << " cells\n";
  ...
  std::cerr
      << "reading configuration grid and initializing configs vector...\n";
  for (int i = 0; i < sz1; ++i) {
    Eigen::Affine3f ps;
      ...
      for (id.k = 0; id.k < size_z; ++id.k) {
        ...
        for (int i = 0; i < sz; ++i) {
          ...
          if (idx < 0 || idx > valid_configs.size()) {
            std::cerr << "something is wrong, config id out of bounds " << idx
                      << std::endl;
            return false;
          }

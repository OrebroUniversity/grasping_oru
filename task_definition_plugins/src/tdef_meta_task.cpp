      ...
      while(j<size){
        ...
        if (type.compare("TDefGeomProj") == 0) {
          ...
          if (prim_type1.compare("line") == 0 && prim_type2.compare("line") == 0) {
            def_ = std::make_shared<
            TDefGeometricAlignment<GeometricLine, GeometricLine> >(this->getGeometricPrimitiveMap(),
             this->getVisualizer());
          } else if (prim_type1.compare("line") == 0 &&
           prim_type2.compare("plane") == 0) {
            def_ = std::make_shared<
    ...
    int TDefMetaTask::update(RobotStatePtr robot_state) {
      ...
      for(auto &task:task_defs_){
        if(task->update(robot_state)!=0){
          printHiqpWarning("Could not update the subtask "+task->getTaskName() +" in TdefMetaTask!");
          return -1;
        }
        e_(i)=task->e_(0);
        J_.row(i)=task->J_;
...
PLUGINLIB_EXPORT_CLASS(hiqp::tasks::TDefMetaTask, hiqp::TaskDefinition)

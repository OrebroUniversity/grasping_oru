...
class SDFTracker : public SimpleOccMapIfce {
 protected:
  ...
  bool quit_;
  // functions
  Eigen::Vector4d VertexInterp(double iso, Eigen::Vector4d &p1d,
 ...
 public:
  ...
  /// with central differences. stepSize chooses how far away from the central
  /// cell the samples should be taken before computing the difference
  virtual double SDFGradient(const Eigen::Vector4d &location, int dim,
                             int stepSize);

  /// Saves the current volume as a VTK image.
  virtual void SaveSDF(
      const std::string &filename = std::string("sdf_volume.vti"));

  /// Loads a volume from a VTK image. The grid is resized to fit the loaded
  /// volume
  ...
  /// Estimates the incremental pose change vector from the current pose,
  /// relative to the current depth map
  virtual Vector6d EstimatePoseFromDepth(void);

  /// Estimates the incremental pose change vector from the current pose,
  /// relative to the current point vector
  virtual Vector6d EstimatePoseFromPoints(void);

  /// Fuses the current depth map into the TSDF volume, the current depth map is
  /// set using UpdateDepth
  virtual void FuseDepth(void);

  /// Fuses the depth map @param depth from a camera with parameters @cam_param
  /// taken from camera pose @param T
  virtual void FuseDepth(cv::Mat &depth, SDF_CamParameters &cam_param,
                         const Eigen::Matrix4d &T);

  /// Fuses the current point vector into the TSDF volume, the current point
  /// vector is set using UpdatePoints
  virtual void FusePoints(void);

  /// Estimates the pose, fuses the given depth map and renders a virtual depth
  /// map (all in one)
  virtual void FuseDepth(const cv::Mat &depth);

  /// Render a virtual depth map. If interactive mode is true (see class
  /// SDF_Parameters) it will also display a preview window with estimated
  /// normal vectors
  virtual void Render(void);

  /// Finds the inverse projection of a 3D point to the image plane, given
  /// camera parameters
  cv::Point2d To2D(const Eigen::Vector4d &location, double fx, double fy,
                   double cx, double cy);

  /// Projects a depth map pixel into a 3D point, given camera parameters.
  Eigen::Vector4d To3D(int row, int column, double depth, double fx, double fy,
                       double cx, double cy);

  /// Computes an antisymmetric matrix based on the pose change vector. To get a
  /// transformation matrix from this, you have to call Twist(xi).exp().
  Eigen::Matrix4d Twist(const Vector6d &xi);

  /// Runs the Marching Tetrahedrons algorithm. The result is then available in
  /// triangles_. Each three consecutive entries in triangles_ represents one
  /// triangle.
  void MakeTriangles(void);

  /// Dumps the zero level set as triangles to an OBJ file. SaveTriangles must
  /// be preceded by a call to MakeTriangles.
  void SaveTriangles(const std::string filename = std::string("triangles.obj"));

  /// Dumps the zero level set as triangles to an STL file. This method also
  /// computes the triangles so there is no need to call MakeTriangles.
  void SaveTrianglesSTL(
      const std::string filename = std::string("triangles.stl"));
  ...
  /// Ray-trace a single ray
  Eigen::Vector3d ShootSingleRay(Eigen::Vector3d &start,
                                 Eigen::Vector3d &direction);

  cv::Mat *GetCurrentDepthImg() { return depthImage_; }
  /// In interactive sessions, this function returns true at any point after a
  /// user has pressed "q" or <ESC> in the render window.
  bool Quit(void);

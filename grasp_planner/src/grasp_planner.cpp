...
GraspPlanner::GraspPlanner(SDF_Parameters &parameters) {
  ...
  nh_.param<std::string>("grasp_frame_name", gripper_frame_name,
                         "planned_grasp");
  ...
  if (!success) {
    ROS_ERROR("could not load gripper constraints file from %s",
              gripper_fname.c_str());
    ros::shutdown();
  }
  ...
  nh_.param<double>("tf_world2sdf_yaw", yaw, 0.0);
  nh_.param<double>("tf_world2sdf_pitch", pitch, 0.0);
  nh_.param<double>("tf_world2sdf_roll", roll, 0.0);

  tf::Matrix3x3 rotationWorld2SDF;
  rotationWorld2SDF.setEulerYPR(yaw, pitch, roll);
  tf::Vector3 translationWorld2SDF(tfx, tfy, tfz);

  tf::Transform tfWorld2SDF;
  tfWorld2SDF.setBasis(rotationWorld2SDF);
  ...
  nh_.param<std::string>("depth_topic_name", depth_topic_name_,
                         "/camera/depth_registered/image");
  nh_.param<std::string>("depth_info_topic_name", depth_info_topic_name_,
                         "/camera/depth_registered/camera_info");
  nh_.param<std::string>("depth_topic_name2", depth_topic_name2_, "none");
  nh_.param<std::string>("depth_info_topic_name2", depth_info_topic_name2_,
                         "none");
  ...
  nh_.getParam("RobustStatisticCoefficient",
               myParameters_.robust_statistic_coefficient);
  ...
  sdf_map_publisher_ =
      nh_.advertise<sdf_tracker_msgs::SDFMap>(sdf_map_topic, 10);
  fused_pc_publisher_ =
  ...
  save_map_server_ =
      nh_.advertiseService("save_map", &GraspPlanner::saveMapCallback, this);
  map_to_edt_ =
      nh_.advertiseService("map_to_edt", &GraspPlanner::mapToEdtCallback, this);
  clear_map_server_ =
      nh_.advertiseService("clear_map", &GraspPlanner::clearMapCallback, this);
  load_volume_server_ = nh_.advertiseService(
  ...
  constraint_pub_ = nh_.advertise<visualization_msgs::MarkerArray>(
      "constraint_marker", 10, true);

  heartbeat_tf_ =
      nh_.createTimer(ros::Duration(0.05), &GraspPlanner::publishTF, this);
...
void GraspPlanner::publishTF(const ros::TimerEvent &event) {
  if (grasp_frame_set) {
    br.sendTransform(tf::StampedTransform(gripper2map, ros::Time::now(),
                                          object_map_frame_name,
                                          gripper_frame_name));
  }
...
void GraspPlanner::depthInfoCallback(
...
void GraspPlanner::depthCallback(const sensor_msgs::Image::ConstPtr &msg) {
  ...
  try {
    tl.waitForTransform(object_map_frame_name, camera_link_, msg->header.stamp,
                        ros::Duration(0.15));
    tl.lookupTransform(object_map_frame_name, camera_link_, msg->header.stamp,
                       camera_frame_to_map);
...
bool GraspPlanner::loadConstraintsCallback(
    grasp_planner::LoadResource::Request &req,
...
bool GraspPlanner::loadVolumeCallback(
    grasp_planner::LoadResource::Request &req,
    grasp_planner::LoadResource::Response &res) {
...
bool GraspPlanner::mapToEdtCallback(
    sdf_tracker_msgs::GetSDFMap::Request &req,
...
bool GraspPlanner::planGraspCallback(grasp_planner::PlanGrasp::Request &req,
  ...
  std::cout << "From frame " << req.header.frame_id << " to "
            << object_map_frame_name << std::endl;
  tf::StampedTransform object_frame_to_map;
  try {
    tl.waitForTransform(object_map_frame_name, req.header.frame_id,
                        ros::Time(0), ros::Duration(1.0));
    tl.lookupTransform(object_map_frame_name, req.header.frame_id, ros::Time(0),
                       object_frame_to_map);
  } catch (tf::TransformException ex) {
    ROS_ERROR("%s", ex.what());
    return false;
  ...
  obj2map =
      obj_fr2map_fr *
      obj2obj_fr;                     //*Eigen::AngleAxisd(M_PI/2,Eigen::Vector3d::UnitX())*Eigen::AngleAxisd(M_PI,Eigen::Vector3d::UnitZ());
  ...
  try {
    tl.waitForTransform(gripper_frame_name, req.approach_frame, ros::Time(0),
                        ros::Duration(1.0));
    tl.lookupTransform(gripper_frame_name, req.approach_frame, ros::Time(0),
                       prototype_in_plan_frame);
  } catch (tf::TransformException ex) {
    ROS_ERROR("%s", ex.what());
    return false;
  }
  ...
  prototype << req.approach_vector[0], req.approach_vector[1],
      req.approach_vector[2];
  ...
  gripper_map->computeValidConfigs(myTracker_, obj2map_f, req.object_radius,
                                   req.object_height, prototype,
                                   req.approach_angle, out);
  tracker_m.unlock();
  ...
  left.name = "left";
  left.type = "plane";
  left.frame_id = grasping_frame;  // TODO: Check this.
  left.visible = false;
  ...
  right.name = "right";
  right.type = "plane";
  right.frame_id = grasping_frame;  // TODO: Check this.
  right.visible = false;
  right.color = {1.0, 0.0, 0.0, 0.5};
  right.parameters = {normal(0), normal(1), normal(2), d + plane_tolerance};

  Eigen::Vector3f zaxis = grasp2global.rotation() * out.inner_cylinder.pose *
  ...
  if (!out.isSphere) {
    ...
    inner.name = "inner";
    inner.type = "cylinder";
    inner.frame_id = grasping_frame;  // TODO: Check this.
    inner.visible = false;
    inner.color = {0.0, 0.0, 1.0, 0.2};
    inner.parameters = {
        zaxis(0), zaxis(1), zaxis(2),  // Axis first.
        grasp2global.translation()(0) +
            out.inner_cylinder.pose.translation()(0),  // Position x
        grasp2global.translation()(1) +
            out.inner_cylinder.pose.translation()(1),  // Position y
        grasp2global.translation()(2) +
            out.inner_cylinder.pose.translation()(2),     // Position z
        out.inner_cylinder.radius_ - cylinder_tolerance,  // radius
        req.object_height};  // height - shouldn't really matter unless the
                             // gripper is "YUGE".
    ...
    outer.name = "outer";
    outer.type = "cylinder";
    outer.frame_id = grasping_frame;
    outer.visible = false;
    outer.color = {0.0, 0.0, 1.0, 0.2};
    outer.parameters = {zaxis(0), zaxis(1), zaxis(2),
                        grasp2global.translation()(0) +
                            out.outer_cylinder.pose.translation()(0),
                        grasp2global.translation()(1) +
                            out.outer_cylinder.pose.translation()(1),
                        grasp2global.translation()(2) +
                            out.outer_cylinder.pose.translation()(2),
    ...
    outer.color = {0.0, 0.0, 1.0, 0.2};
    outer.parameters = {
        grasp2global.translation()(0) + out.outer_sphere.center(0),
    ...
    inner.name = "inner";
    inner.type = "sphere";
    inner.frame_id = grasping_frame;
    inner.visible = false;
    inner.color = {0.0, 0.0, 1.0, 0.2};
    inner.parameters = {
        grasp2global.translation()(0) + out.inner_sphere.center(0),
  ...
  normal = grasp2global.rotation() * out.lower_plane.a;
  d = out.lower_plane.b + normal.dot(grasp2global.translation());
  addPlaneMarker(marker_array, normal.cast<double>(), d + plane_tolerance,
                 req.header.frame_id);
  normal = grasp2global.rotation() * out.upper_plane.a;
  d = out.upper_plane.b + normal.dot(grasp2global.translation());
  addPlaneMarker(marker_array, -normal.cast<double>(), -d - plane_tolerance,
  ...
  addPlaneMarker(marker_array, -out.left_bound_plane.a.cast<double>(),
                 -out.left_bound_plane.b - plane_tolerance, gripper_frame_name);

  addPlaneMarker(marker_array, out.right_bound_plane.a.cast<double>(),
                 out.right_bound_plane.b + plane_tolerance, gripper_frame_name);
  if (!out.isSphere) {
    ...
    addCylinderMarker(
        ...
        out.inner_cylinder.radius_ - cylinder_tolerance, gripper_frame_name,
    ...
    addCylinderMarker(marker_array, Eigen::Vector3d::Zero(),
                      ...
                      gripper_frame_name, req.object_height, "request", 0.8,
                      0.1, 0.1);

  } else {
    addSphereMarker(marker_array, out.inner_sphere.center,
                    out.inner_sphere.radius - cylinder_tolerance,
                    gripper_frame_name);
    addSphereMarker(marker_array, out.outer_sphere.center,
                    out.outer_sphere.radius - cylinder_tolerance,
                    gripper_frame_name);
    addSphereMarker(marker_array, obj2map_f.translation(), req.object_radius,
                    object_map_frame_name, "request", 0.8, 0.1, 0.1);
  }
...
void GraspPlanner::addPlaneMarker(visualization_msgs::MarkerArray &markers,
                                  ...
                                  std::string frame_, std::string namespc,
